				...
				<div class="col-12">
					<div class="card">
						...
						<div class="text-center">
						    <a href="<?= base_url() ?>/print_pbt/<?= $voucher_data['voucher_id'] ?>" class="btn btn-primary">Print Voucher</a>
						</div>
					</div>
				</div>
			</div>
	...
	<script type="text/javascript">
		jquery_3_2_1(document).ready(function () {
			...
			$(document).on('click', '.approve', function(){
				$('#deleteModal').modal('show');
				$('#deleteForm').attr('action', '<?= base_url() ?>/approvepayment/'+<?= $voucher_data['voucher_id'] ?>);
			});

			$(document).on('click', '.decline', function(){
				$('#declineModal').modal('show');
				$('#declineForm').attr('action', '<?= base_url() ?>/declinepayment/'+<?= $voucher_data['voucher_id'] ?>);
			});
		});
	</script>
